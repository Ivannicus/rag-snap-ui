...
- **Overview dashboard, cards or list** — every project in flight with live progress, owners and due dates. A Cards/List toggle switches between detail cards (with a three-band progress wheel) and one-line rows carrying a name, an approved-progress bar, the ready/unanswered/approved counts and the team. The choice is remembered per browser.
